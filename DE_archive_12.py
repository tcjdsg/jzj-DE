...
from calFitness import Fitness, initMess, MTRCA, judgeHuman, allocationHuman, serialGenerationScheme_l, \
    serialGenerationScheme_r, serialGenerationScheme, encoder
from initm import InitM
from utilss import *
from utils.history import History
from utils.logger import Logger
from utils.visualize import export_convergence_chart
...
class JADE():
    ...
    def __init__(self, activities, zaibian: int = 2, cmax: int = 90, psp: float = 1.0, epoch: int = 10000,
                 pop_size: int = 100,
                 M: int = 100,
                 E: float = 0.01,
                 miu_f: float = 0.5,
    ...
    def RUN(self, total_Huamn_resource):
       ...
       # self.allcolate(self.population[0],'l')
    ...
    def allcolate(self, individual,lr):
      ...
      #  MTRCA(stations, individual.codes, self.activities)
        for order in acts:
            ...
            allocationHuman(recoed_h, humans, self.activities, order.id, now_pos)
...
if __name__ == '__main__':
    ...
    # PLS = FixedMes.PLS
    # iter = FixedMes.ge
    file = 'dataset/DE_12_12_instance4.npy'
    epoch = 300
    NP = [50, 100, 150, 200]
    PSP = [0.1, 0.25, 0.5, 0.75]
    M = [20, 40, 60, 80]
    ...
    for i in range(100):
        ...
        for j in range(len(zuhe)):
            ...
            data_psp = None
            for i in range(2):
                model = JADE(activities=FixedMes.act_info, zaibian=zaibian, cmax=cmax, psp=psp, epoch=epoch, pop_size=n,
                             M=m, E=e, miu_f=0.6, miu_cr=0.6, pt=0.1, ap=0.1)
                start = time.time()
