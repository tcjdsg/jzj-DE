...
def rcpsp_solve(jzj_number,order_number,maxtime,act_info,computing_time):
    ...
    sol.start_time = [tasks[i].start for i in range(nbTasks)]
    sol.end_time = [tasks[i].end for i in range(nbTasks)]
    return solver.get_objective_values(),cal,solver.get_solve_status()

instances = np.load(f'dataset/DE_12_12_instance4.npy', allow_pickle=True)
...
for i in range(len(timea)):
        ...
        sol, cal, status = rcpsp_solve(12, 12, 75, FixedMes.act_info, timea[i])
        print("状态------------", status)
        print("目标值------------", sol)
