...
instances = np.load(f'dataset/DE_12_12_instance4.npy', allow_pickle=True)[0]
Init = InitM("dis.csv")
...
for i in range(1):
    print("--------",i,"------------")
    FixedMes.act_info = Init.readData(0, instances)

    FixedMes.total_Huamn_resource = instances[2]
    sol, cal, status = rcpsp_solve(12,12,75,FixedMes.act_info, 60)
...
#     cals.append(cal)
#
# np.save(f'Cplex_8586_8_12.npy', sols)
